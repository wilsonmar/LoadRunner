...
vi_set_URLSource_attribute(){
	...
	// define number of records in file (for all iURLSource_setting options because this is the fall-back):
	    UserIds_file_recs = vi_set_pURLtoShorten_file_recs();
	if( UserIds_file_recs == -1 ){
		lr_error_message("> vi_set_pURLtoShorten_file_recs() UserIds_file_recs=%d",UserIds_file_recs);
	}else{
	  	wi_startPrintingInfo();
		lr_output_message(">> UserIds_file_recs=%d.", UserIds_file_recs);
		wi_resetPrinting();
	}
    ...
    if( iURLSource_setting == 2 ){
		...
		if( rc != LR_PASS ){
	    	...
	    	if( UserIds_file_recs > 0 ){ // if records were found earlier?
				iURLSource_setting = 1; // use local data source
				
				wi_startPrintingInfo();
				lr_output_message(">> rc=%d fall-back to local data source with UserIds_file_recs=%f when iURLSource_setting=%d.", rc, UserIds_file_recs, iURLSource_setting);
				wi_resetPrinting();
			}
	    }else{
